...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="form-group">
            ...
            <input type="text" id="title" name="title" class="form-control" value="{{ old('title', $level->title) }}" required>
            @error('title')
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" class="form-control" rows="2">{{ old('description', $level->description) }}</textarea>
            @error('description')
                <div class="form-error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
            <div class="form-group">
                ...
                <select id="game_type" name="game_type" class="form-control" required>
                    <option value="wordsearch" {{ old('game_type', $level->game_type) == 'wordsearch' ? 'selected' : '' }}>Word Search</option>
                    <option value="crossword" {{ old('game_type', $level->game_type) == 'crossword' ? 'selected' : '' }}>Crossword</option>
                    <option value="wordle" {{ old('game_type', $level->game_type) == 'wordle' ? 'selected' : '' }}>Wordle</option>
                </select>
                @error('game_type')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group" id="gridSizeGroup">
                ...
                <select id="grid_size" name="grid_size" class="form-control">
                    <option value="10" {{ old('grid_size', $level->grid_size) == '10' ? 'selected' : '' }}>10x10 (Small)</option>
                    <option value="15" {{ old('grid_size', $level->grid_size) == '15' ? 'selected' : '' }}>15x15 (Medium)</option>
                    <option value="20" {{ old('grid_size', $level->grid_size) == '20' ? 'selected' : '' }}>20x20 (Large)</option>
                    <option value="25" {{ old('grid_size', $level->grid_size) == '25' ? 'selected' : '' }}>25x25 (Extra Large)</option>
                </select>
                @error('grid_size')
                    <div class="form-error">{{ $message }}</div>
                @enderror
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
            ...
            <div class="form-group">
                ...
                <select id="is_published" name="is_published" class="form-control">
                    <option value="0" {{ old('is_published', $level->is_published) == '0' ? 'selected' : '' }}>Draft</option>
                    <option value="1" {{ old('is_published', $level->is_published) == '1' ? 'selected' : '' }}>Published</option>
                </select>
                @error('is_published')
                    <div class="form-error">{{ $message }}</div>
                @enderror
        ...
        <div class="form-group">
            ...
            @php
                $selectedWords = old('words', $level->words->pluck('id')->toArray());
            @endphp
                        ...
                        <div class="word-item" data-word-length="{{ strlen($word->word) }}" style="margin-bottom: 0.5rem; padding-left: 1rem;">
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="checkbox" class="word-checkbox" name="words[]" value="{{ $word->id }}"
                                    {{ (is_array($selectedWords) && in_array($word->id, $selectedWords)) ? 'checked' : '' }}
            ...
            <div class="form-group" style="margin-top: 2.5rem; padding-top: 2rem; border-top: 1px solid rgba(255, 255, 255, 0.05);">
                <button type="submit" class="btn btn-success" style="margin-right: 1rem;">Update Level</button>
...
<script>
    ...
    // Handle radio button selection for Wordle (sync with hidden checkbox)
